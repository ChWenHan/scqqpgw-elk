...
subroutine gwsefmkhf(ikp,vse,h0)
use modmain
use modgw
use modomp
implicit none
...
complex(8), intent(in) :: h0(nstsv,nstsv)
complex(8), intent(out) :: vse(nstsv,nstsv)
! local variables
integer ik,jk,ist1,ist2,ist3,ist,jst
...
write(*,'("Info(gwsefmkhf): ",I6," of ",I6," k-points")') ikp,nkpt
...
!$OMP END PARALLEL
call freethd(nthd)
!AC and hermitinaze the Seim_C_2_Ser
!
!$OMP CRITICAL(eveqnhf_)
write(fname,'("GWSECI_K",I6.6,"_ISCL",I4.4".OUT")') ikp,iscl
open(50,file=trim(fname),form='formatted')
do ist1=1,nstsv
  do ist2=1,nstsv
    do iw=-nwfm,nwfm,2
      jw=(iw+nwfm)/2
      write(50,'(2I4.3,2I5.3,2G18.10)') ist1,ist2,iw,jw,se(ist1,ist2,jw)
    end do
  end do
end do
close(50)
!$OMP END CRITICAL(eveqnhf_)
!write(*,*)'before se2vsk2 for Se_c'
call se2vsk2(.true.,.true.,ikp,se,vse)
!
!write(*,*)'after se2vsk2 for Se_c'
fpre='HF_GWSER_C'
call writegwclv(ikp,vse,fpre)
fpre='HF_GWSER_X'
call writegwclv(ikp,v,fpre)
!add the local potential mat
! do ist2=1,nstsv
!   do ist1=1,ist2
!     vse(ist1,ist2)=vse(ist1,ist2)+v(ist1,ist2)
!   end do
!   do ist1=ist2+1,nstsv
!     vse(ist1,ist2)=vse(ist1,ist2)+conjg(v(ist2,ist1))
!   end do
! end do
...
if (qprenorm) then
  ...
  do ist=1,nstsv
    do jst=1,nstsv
      vse(ist,jst)=dse(ist,jst)*vse(ist,jst)
    end do
  end do
end if
...
if (mixse) then
  ...
  if (iscl.gt.1) then
    ...
    do ist=1,nstsv
      z00=vse(ist,ist)-vse0(ist,ist)
      dvdiag=dvdiag+sqrt((real(z00)**2+aimag(z00)**2))
      do jst=1,nstsv
        z00=vse(ist,jst)-vse0(ist,jst)
...
!store and write the vse
if (calcgw) then
  call putgwclv(ikp,vse)
  fpre=trim('HF_VSE_MIXED')
  call writegwclv(ikp,vse,fpre)
else
  call putgwclv(ikp,v)
  fpre=trim('HF_GWVFOCK')
  vse=v
  call writegwclv(ikp,v,fpre)
end if
! add the local potential mat
...
!$OMP NUM_THREADS(nthd)
  do iw=0,nwfm
    do ist2=1,nstsv
      do ist1=1,ist2
        se(ist1,ist2,iw)=se(ist1,ist2,iw)+v(ist1,ist2)+h0(ist1,ist2)
      end do
      do ist1=ist2+1,nstsv
        se(ist1,ist2,iw)=se(ist1,ist2,iw)+conjg(v(ist2,ist1))+conjg(h0(ist2,ist1))
      end do
    end do
  end do
!$OMP END PARALLEL DO
