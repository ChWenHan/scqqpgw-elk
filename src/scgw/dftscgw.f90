...
!integer nthd
real(8) dv,etp,de,timetot,sumkdvse,sumkdvsediag
! allocatable arrays
real(8), allocatable :: work(:)
real(8), allocatable :: vmt(:,:),vir(:)
real(8), allocatable :: bmt(:,:,:),bir(:,:)
!complex(8), allocatable :: evecsv(:,:)
!complex(8), allocatable :: vse(:,:)
! initialise global variables
call init0
call init1
call init2
call init3
allocate(dvse(nkpt,maxsclgw))
allocate(dvsediag(nkpt,maxsclgw))
!hfgw=.false.
...
! write interatomic distances to file
  ...
  close(50)
! open INFO.OUT file
  open(60,file='GWDFT_INFO'//trim(filext),form='FORMATTED')
...
!whch---- output some gw vars. see GW_**
call writegwvars
inquire(file='EPSINV.OUT',exist=exist)
! generate the inverse dielectric function and write to file
!allocate the locate potential
...
! generate the first- and second-variational eigenvectors and eigenvalues
  call genevfsv
!regenerate the occsv, may read in from previous gnd
  if (iscl.eq.1) then
    call occupy
  end if
  do ik=1,nkpt
...
! generate epsinv
  if (iscl.eq.1) then 
    if (.not.readepsinv) then
      if (mp_mpi) write(*,*)'init EPSINV.OUT'
      do ik=1,nkpt
      ...
      call genpmat
      call epsinv
    elseif ((readepsinv).and.(exist)) then
      if (mp_mpi) write(*,*)'EPSINV.OUT exist, read current epsinv'
    else
      if (mp_mpi) write(*,*)'EPSINV.OUT does not exist or not read epsinv'
    end if
  end if
  if ((fscgw.eq.1).and.(iscl.gt.1)) then
    if (mp_mpi) write(*,'("gen EPSINV.OUT for iscl:",  I6.3) ')iscl
    call genpmat
    call epsinv
  end if
!--------------------------------------------
! for -V_xc
  if (hfgw) then
    write(*,'("Sloving 3rd variational-eq with GW se in HF form for iscl : ",I6.3)') iscl
    call hflocal(vmt,vir,bmt,bir)
    call genkmat(.true.,.true.)
    call eveqnhfgw(vmt,vir,bmt,bir)
  else
    write(*,'("Sloving 3rd variational-eq with GW se in DFT form for iscl : ",I6.3)') iscl
    call gwlocal(vmt,vir,bmt,bir)
    call eveqngwdft(vmt,vir,bmt,bir)
  end if
  !call hflocal(vmt,vir,bmt,bir)
  !write(*,*) 'after gwlocal'
! generate matrix format Vse=Σxc-Vxc
  !call gengwsemat(vmt,vir,bmt,bir)
  !call genkmat(.true.,.true.)
! do the gw loop with H=H0+Vxc+Σxc-Vxc\
  !write(*,'("Sloving 3rd variational-eq with GW se for iscl : ",I6.3)') iscl
  !call eveqnhfgw(vmt,vir,bmt,bir)
  
! find the occupation numbers and Fermi energy
  if (dmatfromseiw) then
    call gwefermi
    call gwrhomag
    if (mp_mpi) then
...
! compute the Kohn-Sham potentials and magnetic fields
  call potks(.true.)
  !generate vse in 'r' space
  !if (tvse) call vsek2mesh
  call mpi_barrier(mpicom,ierror)
  if (mp_mpi) then
...
! compute the energy components
  ...
  if (mp_mpi) then
    if (iscl.gt.1) then
      ...
      sumkdvsediag=0.d0
      do ik=1,nkpt
        sumkdvse=(dvse(ik,iscl)*wkpt(ik))/(nstsv)+sumkdvse
        sumkdvsediag=dvsediag(ik,iscl)*wkpt(ik)+sumkdvsediag
      end do
      write(60,'("magnitde of residu of Vse is (dvse/nstsv) : ",G18.10)') sumkdvse
      write(60,'("magnitde of residu of diag-Vse is(dvsediag)) : ",G18.10)') sumkdvsediag
      write(60,'("magnitde of residu of diag-Vse/nstsv is(dvsediag/nstsv) : ",G18.10)') sumkdvsediag/nstsv
    end if  
  end if
  if (mp_mpi) then
...
! write STATE.OUT file if required
    if (twrite.or.((nwrite.ge.1).and.(mod(iscl,nwrite).eq.0))) then
      call writestate
      write(60,*)
...
if (mp_mpi) then
  ...
  write(60,'("+------------------------------+")')
! write density and potentials to file only if maxsclgw > 1
  if (maxsclgw.gt.1) then
    call writestate
    write(60,*)
    write(60,'("Wrote STATE.OUT")')
