...
subroutine writegwviw(ik,nw,wf,v,fpre)
use modmain
use modomp
use modgw
implicit none
! arguments
integer, intent(in) :: ik,nw
complex(8), intent(in) :: wf(nw), v(nstsv,nstsv,nw)
...
if ((twrite).or.(nwrite.ge.1).and.(mod(iscl,nwrite).eq.0).or.(iscl.eq.1)) then
  ...
  do ist1=1,nstsv
    do ist2=1,nstsv
      do iw=1,nw
        write(98,'(2I6.3,I5.3,2G18.10)') ist1,ist2,iw,v(ist1,ist2,iw)
      end do
    end do
  end do
  close(98)
