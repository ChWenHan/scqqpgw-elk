...
logical ramdisk
! maximum allowed number of files in the RAM disk
integer, parameter, private :: maxfiles=20
! maximum number of records per file
integer, parameter, private :: maxrec=10000
...
! this subroutine should be called from a critical section
implicit none
! arguments
character(*), intent(in) :: fname
...
if (present(v1)) then
  call rvcopy(3,v1,file(fnum)%rec(irec)%dat(i))
  i=i+6
end if
if (present(v2)) then
  call rvcopy(3,v2,file(fnum)%rec(irec)%dat(i))
  i=i+6
end if
if (present(rva)) then
  call rvcopy(nrv,rva,file(fnum)%rec(irec)%dat(i))
  i=i+2*nrv
end if
if (present(zva)) then
  call rvcopy(2*nzv,zva,file(fnum)%rec(irec)%dat(i))
end if
end subroutine
...
if (present(v1)) then
  if (n.lt.6) return
  call rvcopy(3,file(fnum)%rec(irec)%dat(i),v1)
  i=i+6
  n=n-6
end if
if (present(v2)) then
  if (n.lt.6) return
  call rvcopy(3,file(fnum)%rec(irec)%dat(i),v2)
  i=i+6
  n=n-6
end if
if (present(rva)) then
  if (n.lt.2*nrv) return
  call rvcopy(nrv,file(fnum)%rec(irec)%dat(i),rva)
  i=i+2*nrv
  n=n-2*nrv
end if
if (present(zva)) then
  if (n.lt.4*nzv) return
  call rvcopy(2*nzv,file(fnum)%rec(irec)%dat(i),zva)
end if
